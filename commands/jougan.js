...
module.exports.run = async (bot, message, args) => {
    ...
    if (member.roles.cache.some(role => role.name === 'Jougan'))  {
            ...
            Data.findOne({
                ...
                Cooldown.findOne({
                    ...
                    if(!cooldown) {
                        ...
                        embed.setDescription(`You used your Jougan against some enemies and got ${reward} Ryo!`)
                            embed.setColor("#40E0D0");
                        return message.channel.send(embed);
                    } else {
                        if(timeout -(Date.now() - cooldown.Jougan) > 0) {
                            let time = ms(timeout - (Date.now() - cooldown.Jougan));
