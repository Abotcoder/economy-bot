...
module.exports.run = async (bot, message, args) => {
    ...
    if (member.roles.cache.some(role => role.name === 'Rinne Sharingan'))  {
            ...
            Data.findOne({
                ...
                Cooldown.findOne({
                    ...
                    if(!cooldown) {
                        ...
                        embed.setDescription(`You used your Rinne Sharingan to cast the infinite tsukiyomi on some enemies and got ${reward} Ryo!`)
                            embed.setColor("#40E0D0");
                        return message.channel.send(embed);
                    } else {
                        if(timeout -(Date.now() - cooldown.RinneSharingan) > 0) {
                            let time = ms(timeout - (Date.now() - cooldown.RinneSharingan));
                            ...
                            data.money += reward;
                            cooldown.RinneSharingan = Date.now();
                            data.save().catch(err => console.log(err));
