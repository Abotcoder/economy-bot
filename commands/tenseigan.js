...
module.exports.run = async (bot, message, args) => {
    ...
    if (member.roles.cache.some(role => role.name === 'Tenseigan'))  {
            ...
            Data.findOne({
                ...
                Cooldown.findOne({
                    ...
                    if(!cooldown) {
                        ...
                        embed.setDescription(`You activated your Tenseigan Chakra Mode and got ${reward} Ryo!`)
                            embed.setColor("#40E0D0");
                        return message.channel.send(embed);
                    } else {
                        if(timeout -(Date.now() - cooldown.Tenseigan) > 0) {
                            let time = ms(timeout - (Date.now() - cooldown.Tenseigan));
