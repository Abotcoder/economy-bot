...
module.exports.run = async (bot, message, args) => {
    ...
    let ryoreward = Math.floor(Math.random() * 200) + 300;
    let timeout = 86400000;
    let reward = ryoreward;
    let embed = new Discord.MessageEmbed();

    let member = message.mentions.members.first() || message.member;
    if (member.roles.cache.some(role => role.name === 'Particle Release'))  {
                ...
                Cooldown.findOne({
                    ...
                    if(!cooldown) {
                        ...
                        data.save().catch(err => console.log(err));
                            ...
                            embed.setDescription(`You used your particle style in a mission and completed it with a reward of ${reward} Ryo!`)
                            embed.setColor("#40E0D0");
                        return message.channel.send(embed);
                    } else {
                        if(timeout -(Date.now() - cooldown.ParticleStyle) > 0) {
                            let time = ms(timeout - (Date.now() - cooldown.ParticleStyle));
                            ...
                            data.money += reward;
                            cooldown.ParticleStyle = Date.now();
                            data.save().catch(err => console.log(err));
